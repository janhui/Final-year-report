...
One of the main drawbacks of the project is the lack of resources on the device. We used a nexus 7 as our device for the CameraApp, which is a relatively powerful device, out of all the options of android devices. It ranked as the 5\textsuperscript{th} best android tablet in the world\cite{techradar-android-tablets}. However we were only able to get a refreshing rate of \textless 1 Frames per second (fps). However we believe this could be improved by taking advantage of the multi-core of the device. Table \ref{refresh_rate_table} shows the improvement in the refresh rate after the optimisation. We would have also liked to investigate the application on a Samsung Tab S which has a much better resources which would definitely improve the refresh rate. Table \ref{spec_comparison} shows the difference in resource between the Nexus 7 and Samsung Tab S
\begin{table}[h]
    \centering
    \begin{tabular}{|c|c|c|}
        \hline 
...
Another improvement we could have implemented was to create a C++ class that encapsulate all of the OpenCV processes that we used. This would decrease the number of JNI calls we had to make. Currently we are using around 32 calls to the OpenCV library, which means there are around 64 calls in the single thread the single thread application. We would have been able to decrease that to just 2 by creating the C++ class. It would be something something interesting to evaluate, and see how much difference it would have made to our refresh rate. However that will be one of the future work we will discuss about.
